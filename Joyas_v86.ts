...
// ═══════════════════════════════════════════════════════════
// ROMET JOYERÍA — Edge Function v86 (correo master a usuario conectado y propietario)
...
const GLOSARIO_TERMINOS: GlosarioItem[] = [
  ...
  },
  {
    ...
    patrones: [/\bgalerias?\b/i, /\bcalad[oas]{1,2}\b/i, /\bcanastillas?\b/i, /\bopenwork\s+gallery\b/i],
...
// ── CADENAS Y TIPOLOGÍAS DE PULSERA (DOCUMENTO DE CADENAS Y PULSERAS) ───
  {
    id: "tubogas",
    termino: "tubogas",
    patrones: [/\btubogas\b/i, /\bgas[\s-]pipe\b/i],
    definicion: "TUBOGAS: a flexible, continuous tubular metallic band formed by interlocking, coiled gold strips with smooth rounded outer ribs, wrapping seamlessly without solder or clasps (iconic Bvlgari Tubogas heritage style)."
  },
  {
    id: "singapur",
    termino: "cadena singapur",
    patrones: [/\bsingapur\b/i, /\bsingapore\s+chain\b/i],
    definicion: "CADENA SINGAPUR (Singapore chain): diamond-cut curb links twisted into a fluid, delicate continuous helical spiral, catching light with dynamic undulating shimmer."
  },
  {
    id: "espina_pez",
    termino: "espina de pez / herringbone",
    patrones: [/\bespina\s+de\s+pez\b/i, /\bherringbone\b/i],
    definicion: "ESPINA DE PEZ / HERRINGBONE (herringbone chain): flat, ultra-smooth flexible ribbon formed by miniature parallel V-shaped gold plates aligned in a woven chevron pattern with mirror-like liquid reflection. Strictly lays flat against the skin."
  },
  {
    id: "cola_raton",
    termino: "cola de ratón",
    patrones: [/\bcola\s+de\s+rat[oó]n\b/i, /\bmouse\s+tail\b/i],
    definicion: "COLA DE RATÓN (mouse tail / tubular snake chain): ultra-fine, smooth seamless cylindrical tube chain with a round cross-section and silky uninterrupted fluid drape. NOT a literal mouse tail."
  },
  {
    id: "cola_zorro",
    termino: "cola de zorro",
    patrones: [/\bcola\s+de\s+zorro\b/i, /\bfoxtail\b/i],
    definicion: "COLA DE ZORRO (foxtail chain): dense, architectural 3D four-sided square braided weave of interlocking Chevron loops, forming a substantial, flexible woven precious metal cord. NOT a literal fox tail."
  },
  {
    id: "bizantina",
    termino: "cadena bizantina / rey",
    patrones: [/\bbizantina\b/i, /\bbyzantine\b/i, /\bcadena\s+rey\b/i, /\bking\s+chain\b/i],
    definicion: "CADENA BIZANTINA / REY (Byzantine / King chain): complex interlocking woven rope of double circular rings folding backward and forward into intricate decorative geometric links with substantial mass and regal medieval texture."
  },
  {
    id: "paperclip",
    termino: "cadena paperclip / clip",
    patrones: [/\bpaperclip\b/i, /\bpaper\s+clip\b/i, /\beslabones?\s+alargados?\b/i, /\btrombone\s+link\b/i],
    definicion: "CADENA PAPERCLIP (paperclip chain): contemporary openwork links featuring clean, elongated rectangular/oval wire loops with rounded corners and minimal modern airy spacing."
  },
  {
    id: "malla_milanesa",
    termino: "malla milanesa",
    patrones: [/\bmilanesa\b/i, /\bmalla\s+milanesa\b/i, /\bmilanese\s+mesh\b/i],
    definicion: "MALLA MILANESA (Milanese mesh): fine, dense fabric-like metallic textile woven from micro-gauge precious gold wire spirals, giving silky fluid flexibility and high-end satin drape."
  },
  {
    id: "marinera",
    termino: "cadena marinera / gucci",
    patrones: [/\bmarinera\b/i, /\banchor\s+chain\b/i, /\bmariner\s+chain\b/i, /\beslab[oó]n\s+gucci\b/i, /\bpuff\s+mariner\b/i],
    definicion: "CADENA MARINERA / GUCCI (Anchor / Mariner / Puff Mariner chain): repeating oval links bisected by a crisp horizontal dividing crossbar through the center, inspired by nautical anchor chains. If puff mariner, features pillowy rounded domed links."
  },
  {
    id: "pulsera_tenis",
    termino: "pulsera tenis",
    patrones: [/\b(?:pulsera|brazalete)s?\s+tenis\b/i, /\btennis\s+bracelet\b/i],
    definicion: "PULSERA TENIS (diamond tennis bracelet): symmetrical, continuous flexible single-line row of individually articulated four-prong collets, each holding a matched brilliant-cut diamond, secured by an integrated box clasp with safety latch."
  },
  {
    id: "pulsera_riviere",
    termino: "pulsera rivière",
    patrones: [/\brivi[eè]re\b/i, /\bcolli?er\s+rivi[eè]re\b/i, /\bpulsera\s+rivi[eè]re\b/i],
    definicion: "PULSERA / COLLAR RIVIÈRE: seamless high-jewelry stream ('river') of uniform or gently graduated faceted gemstones mounted in individual openwork low-profile gallery collets with virtually invisible joints."
  },
  {
    id: "brazalete_bangle",
    termino: "brazalete rígido / bangle",
    patrones: [/\b(?:brazaletes?|pulseras?)\s+r[ií]gid[oas]{1,2}\b/i, /\bbangle\b/i, /\baro\s+r[ií]gido\b/i],
    definicion: "BRAZALETE RÍGIDO / BANGLE (rigid bangle): solid, circular or oval rigid hoop of structured precious metal with no flex, retaining its bold sculptural form."
  },
  {
    id: "brazalete_cuff",
    termino: "brazalete abierto / cuff",
    patrones: [/\b(?:brazaletes?|pulseras?)\s+abiert[oas]{1,2}\b/i, /\bcuff\b/i, /\bbrazalete\s+de\s+abertura\b/i],
    definicion: "BRAZALETE ABIERTO / CUFF (open cuff bracelet): wide, sculptural semi-rigid metal bracelet featuring an open gap on the underside or side, with terminal caps."
  },
  {
    id: "esclava",
    termino: "esclava / ID bracelet",
    patrones: [/\besclavas?\b/i, /\bid\s+bracelet\b/i, /\bpulsera\s+con\s+placa\b/i],
    definicion: "ESCLAVA / ID BRACELET: robust curb or Cuban link chain featuring a central flat, polished rectangular curved metal identification bar plate."
  },
  {
    id: "pulsera_bolo",
    termino: "pulsera bolo",
    patrones: [/\bbolo\s+bracelet\b/i, /\bpulseras?\s+bolo\b/i, /\bpulseras?\s+ajustable[s]?\b/i, /\bcierre\s+deslizante\b/i],
    definicion: "PULSERA BOLO (adjustable slider / bolo bracelet): chain bracelet whose two ends feed through a polished spherical bead slider with silicone friction grip, finishing in decorative metal droplets."
  },

  // ── TALLAS CLÁSICAS DE DIAMANTES Y GEMAS (SILUETAS Y RATIOS) ───────────
  {
    id: "talla_princesa",
    termino: "talla princesa",
    patrones: [/\btalla\s+princesa\b/i, /\bprincess\s+cut\b/i],
    definicion: "TALLA PRINCESA (princess cut): brilliant square faceted cut with sharp, crisp 90-degree pointed corners and an inverted pyramid pavilion delivering intense fire and kaleidoscopic sparkle."
  },
  {
    id: "talla_esmeralda",
    termino: "talla esmeralda",
    patrones: [/\btalla\s+esmeralda\b/i, /\bemerald\s+cut\b/i],
    definicion: "TALLA ESMERALDA (emerald cut): rectangular step-cut gemstone featuring beveled diagonal corners and clean parallel rectilinear facets producing an elegant 'hall of mirrors' reflection."
  },
  {
    id: "talla_marquesa",
    termino: "talla marquesa",
    patrones: [/\btalla\s+marquesa\b/i, /\bmarquise\s+cut\b/i, /\bnavette\b/i],
    definicion: "TALLA MARQUESA / NAVETTE (marquise cut): elongated football-shaped or boat-shaped brilliant cut with curved convex flanks tapering to two sharp pointed tips protected by V-prong caps."
  },
  {
    id: "talla_cojin",
    termino: "talla cojín / cushion",
    patrones: [/\btalla\s+coj[ií]n\b/i, /\bcushion\s+cut\b/i],
    definicion: "TALLA COJÍN / CUSHION CUT: squarish or rectangular gemstone with softly rounded pillow-like corners, deep pavilion, and expansive brilliant facets radiating antique romantic luster."
  },
  {
    id: "talla_pera",
    termino: "talla pera / lágrima",
    patrones: [/\btalla\s+(?:pera|l[aá]grima|gota)\b/i, /\bpear\s+cut\b/i, /\bteardrop\s+cut\b/i],
    definicion: "TALLA PERA / LÁGRIMA (pear / teardrop cut): teardrop brilliant silhouette combining a rounded semicircular base tapering smoothly into a delicate single pointed tip."
  },
  {
    id: "talla_asscher",
    termino: "talla asscher",
    patrones: [/\basscher\b/i, /\btalla\s+asscher\b/i],
    definicion: "TALLA ASSCHER (Asscher cut): square vintage step cut with deeply clipped octagonal corners, a high crown, and layered square step facets creating an iconic windmill pattern in the center."
  },
  {
    id: "talla_baguette",
    termino: "talla baguette",
    patrones: [/\bbaguettes?\b/i, /\btalla\s+baguette\b/i],
    definicion: "TALLA BAGUETTE: slim, elongated rectangular step-cut diamond with clean 90-degree corners, perfectly parallel facets, and geometric transparency."
  },
  {
    id: "talla_trillon",
    termino: "talla trillón / trilliant",
    patrones: [/\btrill[oó]n\b/i, /\btrilliant\b/i, /\btalla\s+triangular\b/i],
    definicion: "TALLA TRILLÓN (trillion / trilliant cut): equilateral triangular brilliant cut with sharp or softly curved equilateral sides and a lively starburst facet arrangement."
  },
  {
    id: "talla_radiante",
    termino: "talla radiante",
    patrones: [/\bradiante\b/i, /\bradiant\s+cut\b/i],
    definicion: "TALLA RADIANTE (radiant cut): rectangular or square cut with cropped corners matching an emerald cut silhouette, but engineered with a brilliant 70-facet pavilion providing vibrant fire."
  },

  // ── GEMAS RARAS Y FENÓMENOS ÓPTICOS (GLOSARIO DE 57 GEMAS) ───────────
  {
    id: "piedra_lunar",
    termino: "piedra lunar / adularescencia",
    patrones: [/\bpiedra\s+lunar\b/i, /\bmoonstone\b/i, /\badularescen(?:cia|te)\b/i],
    definicion: "PIEDRA LUNAR (moonstone with adularescence): translucent, milky bluish-white feldspar gem displaying adularescence — a floating, billowy ethereal electric-blue shimmer billowing beneath the polished cabochon dome surface as light moves."
  },
  {
    id: "opalo",
    termino: "ópalo / juego de color",
    patrones: [/\b[oó]palo[s]?\b/i, /\bopal\b/i, /\bjuego\s+de\s+color\b/i, /\bplay[\s-]of[\s-]color\b/i],
    definicion: "ÓPALO (precious opal with play-of-color): translucent to luminous milk/black matrix exploding with vivid holographic rainbow diffraction pinfire flashes (fiery red, neon green, cyan, and gold) shifting dynamically with viewpoint."
  },
  {
    id: "labradorita",
    termino: "labradorita / labradorescencia",
    patrones: [/\blabradorita\b/i, /\blabradorite\b/i, /\blabradorescen(?:cia|te)\b/i, /\bespectrolita\b/i],
    definicion: "LABRADORITA (labradorite with labradorescence): smoky dark gray mineral bursting into vivid peacock-blue, electric cyan, and iridescent bronze-gold metallic spectral flashes when reflecting light."
  },
  {
    id: "ojo_tigre",
    termino: "ojo de tigre / chatoyancia",
    patrones: [/\bojo\s+de\s+tigre\b/i, /\btiger'?s?\s+eye\b/i, /\bchatoyan(?:cia|te)\b/i],
    definicion: "OJO DE TIGRE (tiger's eye with chatoyancy): rich golden-brown and caramel silky quartz exhibiting strong chatoyancy — a sharp, luminous sliding cat's-eye light slit gliding smoothly across the curved surface."
  },
  {
    id: "tanzanita",
    termino: "tanzanita",
    patrones: [/\btanzanita\b/i, /\btanzanite\b/i],
    definicion: "TANZANITA (tanzanite): intensely saturated royal velvet blue to deep midnight violet gemstone exhibiting pronounced pleochroism, with flashes of rich indigo, amethyst purple, and subtle burgundy under warm light."
  },
  {
    id: "alejandrita",
    termino: "alejandrita",
    patrones: [/\balejandrita\b/i, /\balexandrite\b/i, /\bcambio\s+de\s+color\b/i],
    definicion: "ALEJANDRITA (Alexandrite color change): rare chrysoberyl displaying dramatic optical color change — lush moss-to-emerald green in cool daylight, transforming into rich raspberry red or purple-ruby under warm incandescent candle lighting."
  },
  {
    id: "turmalina_paraiba",
    termino: "turmalina paraíba",
    patrones: [/\bpara[ií]ba\b/i, /\bturmalina\s+para[ií]ba\b/i],
    definicion: "TURMALINA PARAÍBA (Paraíba tourmaline): world-renowned hyper-saturated electric neon turquoise-cyan to mint green gemstone with intense glowing internal luminescence driven by copper traces."
  },
  {
    id: "morganita",
    termino: "morganita",
    patrones: [/\bmorganita\b/i, /\bmorganite\b/i],
    definicion: "MORGANITA (morganite): transparent beryl of refined soft blush pink, rose-petal, or warm peachy pastel hues, exceptionally clear with romantic diamond-like brilliance."
  },

  // ── FORNITURAS Y MECANISMOS DE SUSPENSIÓN (CATÁLOGO DE FORNITURAS) ─────
  {
    id: "asa_suspension",
    termino: "asa de suspensión / reasa de dije",
    patrones: [/\basa\s+de\s+suspensi[oó]n\b/i, /\breasa\s+de\s+dije\b/i, /\bpendant\s+bail\b/i],
    definicion: "ASA DE SUSPENSIÓN / REASA DE DIJE (pendant bail): elegant tapered loop attached to the top of a pendant or cross, engineered with a smooth internal aperture allowing the chain to slide freely without snagging. NOT a bucket handle."
  },
  {
    id: "asa_pellizco",
    termino: "asa de pellizco",
    patrones: [/\basa\s+de\s+pellizco\b/i, /\bpinch\s+bail\b/i],
    definicion: "ASA DE PELLIZCO (pinch bail): flexible precious metal clip whose two opposing tapered pegs seat securely into the side-drilled holes of a briolette or gemstone drop."
  },
  {
    id: "casquillo_espigo",
    termino: "casquillo con espigo",
    patrones: [/\bcasquillo\s+con\s+espigo\b/i, /\bcup\s+and\s+peg\b/i, /\bcazoleta\s+con\s+perno\b/i],
    definicion: "CASQUILLO CON ESPIGO (cup & peg mounting): hemispherical or fluted concave metal cup fitted with a rigid vertical pin post that inserts cleanly into a half-drilled spherical pearl or bead for secure, seamless mounting."
  }
];
...
${reglasRender}`;
    ...
    console.log(`v84 — mode: ${modo}, model: ${GEMINI_MODEL}, hasRefImage: ${!!imagenParaGemini}`);
    ...
    if (RESEND_API_KEY) {
      ...
      const ownerHtml = `<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;">
        ...
        <table style="width:100%;border-collapse:collapse;margin-top:16px;">
          ...
          <tr><td style="padding:8px;border-bottom:1px solid #eee;"><strong>Email de contacto</strong></td><td style="padding:8px;border-bottom:1px solid #eee;">${email || ""}</td></tr>
          ${user?.email ? `<tr><td style="padding:8px;border-bottom:1px solid #eee;"><strong>Usuario conectado</strong></td><td style="padding:8px;border-bottom:1px solid #eee;">${user.email}</td></tr>` : ""}
      ...
      // ── Enviar correo master al propietario y al usuario autenticado ──
      const adminRecipients: string[] = [];
      if (PROPIETARIO_EMAIL) {
        adminRecipients.push(PROPIETARIO_EMAIL);
      }
      if (user?.email && !adminRecipients.some(e => e.toLowerCase() === user.email.toLowerCase())) {
        adminRecipients.push(user.email);
      }
      ...
      for (const recipient of adminRecipients) {
        try {
          ...
          console.log(`Master email sent to ${recipient}, status:`, ownerRes.status);
          // Pausa preventiva de 550ms para respetar el rate limit de Resend (2 req/s)
          await new Promise((resolve) => setTimeout(resolve, 550));
        } catch(e) {
          console.error(`Master email error for ${recipient}:`, e);
        }
      }

      // ── Enviar correo de confirmación al cliente (email de contacto o usuario) ──
      const clientEmailTarget = email || user?.email;
      if (clientEmailTarget) {
        ...
        try {
          ...
          console.log(`Client email sent to ${clientEmailTarget}, status:`, clientRes.status);
        } catch(e) {
    ...
    console.error("v86 ERROR:", error.message);
    return new Response(
