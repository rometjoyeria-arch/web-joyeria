			...
			<div class="max-w-3xl mx-auto px-4 py-12 text-center" style="animation: fadeInUp 0.8s ease-out;">
				<div class="mb-8">
					...
					<p class="text-muted-foreground font-sans tracking-wide max-w-lg mx-auto text-lg">
						...
						We will contact you soon at <strong>${state.email}</strong>.
					</p>
				</div>
				...
				<div class="bg-white p-8 rounded-2xl shadow-lg border border-border/50 mb-8"
					...
					<div id="imagen-generada">
						<p class="text-muted-foreground font-sans">
							Your design is being processed. You will receive an email with the result shortly.
						</p>
					</div>
				</div>`}
				...
				<div id="cambios-panel" style="display:none;" class="bg-white p-6 rounded-2xl shadow-lg border border-border/50 mb-6 text-left">
					...
					<textarea id="cambios-texto"
					          ...
					          style="min-height: 280px !important; height: 280px !important;">[Edit this template to describe your changes so the AI can execute them precisely]
...
- PIECE TYPE: (e.g., Change from a pendant to earrings)</textarea>
					...
					<button id="redesign-btn"
					        onclick="(function(){
					        	...
					        	if (!cambios || cambios.startsWith('[Edit this template')) {
				...
				<div class="bg-white p-6 rounded-2xl shadow-lg border border-border/50 text-left mb-8"
					...
					<div class="grid grid-cols-2 gap-4 font-sans text-sm">
						...
						${state.budget ? `<div><span class="text-muted-foreground">Budget:</span> <strong>${state.budget}€</strong></div>` : ''}
						${state.size ? `<div><span class="text-muted-foreground">Size:</span> <strong>${state.size}</strong></div>` : ''}
					</div>
				</div>
				...
				<div style="max-width: 400px; margin: 0 auto; animation: fadeInUp 0.8s ease-out 0.6s both;">
					...
					<div id="prototipado-modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.8); z-index:9999; align-items:center; justify-content:center; padding:20px; animation:fadeIn 0.3s ease;">
						<div style="background:#fff; border-radius:16px; max-width:500px; width:100%; padding:30px; position:relative; text-align:center; box-shadow:0 10px 40px rgba(0,0,0,0.2);">
							...
							<h3 style="font-family:'Cormorant Garamond',serif; font-size:1.8rem; margin-bottom:10px; color:#000; text-transform:none; letter-spacing:normal;">Choose your option</h3>
							<p style="font-family:sans-serif; font-size:0.9rem; color:#666; margin-bottom:24px; text-transform:none; letter-spacing:normal;">How would you like to proceed with your design?</p>
							...
							<div style="display:flex; flex-direction:column; gap:12px; font-family:sans-serif;">
								...
								<button onclick="window.requestQuote('C', 'en', this)" style="background:#f5f5f5; color:#000; border:1px solid #ddd; padding:16px; border-radius:8px; cursor:pointer; font-size:1rem; font-weight:500; transition:all 0.2s;">
									Prototyped and cast model (silver or gold)
								</button>
								<button onclick="window.requestQuote('D', 'en', this)" style="background:#f5f5f5; color:#000; border:1px solid #ddd; padding:16px; border-radius:8px; cursor:pointer; font-size:1rem; font-weight:500; transition:all 0.2s;">
